...
export function updateArticle(data: ArticleForm): AxiosPromise<Result<null>> {
  ...
  const convertedData = {
    ...
    description: data.articleDesc,
    cover: data.articleCover,
    categoryId: data.categoryId,
    isOriginal: data.articleType === 1,
    originalUrl: data.articleType !== 1 ? data.originalUrl : undefined,
    isTop: data.isTop === 1,
    isPublish: data.status !== 3, // 如果status不是3(草稿)，则为发布状态
